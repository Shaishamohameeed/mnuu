...
function setupEventListeners() {
    ...
    // Search functionality - التصحيح هنا
...
// Search functions - التصحيح الكامل هنا
...
function performSearch() {
    ...
    categories.forEach(category => {
        ...
        // إظهار/إخفاء القسم بناءً على وجود نتائج
        category.style.display = categoryHasResults ? 'block' : 'none';
        
        // إضافة تأثير للقسم الذي يحتوي على نتائج
        if (categoryHasResults) {
            category.classList.add('has-search-results');
        } else {
            category.classList.remove('has-search-results');
        }
    });
    ...
    if (foundItems === 0 && query !== '') {
        ...
        // إذا كانت هناك نتائج، التمرير إلى أول نتيجة
        if (foundItems > 0) {
            const firstResult = document.querySelector('.menu-item[style="display: block"]');
            if (firstResult) {
                const category = firstResult.closest('.category');
                if (category) {
                    const headerHeight = document.querySelector('header').offsetHeight;
                    const targetPosition = category.offsetTop - headerHeight - 20;
...
function showNoResultsMessage(query) {
    ...
    message.innerHTML = `
        <div style="text-align: center; padding: 3rem; background: rgba(255,255,255,0.95); border-radius: 20px; margin: 2rem; box-shadow: 0 5px 15px rgba(0,0,0,0.1);">
            <i class="fas fa-search" style="font-size: 3rem; color: #803a7a; margin-bottom: 1rem;"></i>
            <h3 style="color: #803a7a; margin-bottom: 0.5rem; font-size: 1.5rem;">لم نجد ما تبحث عنه</h3>
            <p style="color: #666; margin-bottom: 1rem;">لا توجد وجبات مطابقة لـ "<strong style="color: #803a7a;">${query}</strong>"</p>
            <p style="color: #999; font-size: 0.9rem;">جرب البحث باسم وجبة مختلف أو تصفح الأقسام</p>
        </div>
    `;
...
// الموقع على خرائط جوجل - الدالة المعدلة
function openLocation() {
    closeSidebarFunc();
    
    // رابط خرائط جوجل المطلوب
    const mapUrl = 'https://maps.app.goo.gl/NLzQn6b1Fs2bDGTd8';
    
    // فتح الرابط في نافذة جديدة
    window.open(mapUrl, '_blank');
    
    // تتبع الحدث لأغراض التحليل
    if (typeof trackEvent === 'function') {
        trackEvent('location_opened', {
            action: 'google_maps_opened',
            url: mapUrl
        });
    }
}
    ...
    loader.innerHTML = `
        <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255,255,255,0.9); display: flex; justify-content: center; align-items: center; z-index: 9999;">
            <div style="text-align: center;">
                <div style="width: 50px; height: 50px; border: 3px solid #f3f3f3; border-top: 3px solid #803a7a; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 1rem;"></div>
                <p style="color: #803a7a; font-weight: 600;">جاري التحميل...</p>
            </div>
        </div>
        <style>
            @keyframes spin {
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    installBanner.innerHTML = `
        <div style="position: fixed; bottom: 20px; left: 20px; right: 20px; background: linear-gradient(45deg, #803a7a, #5a2a5a); color: white; padding: 1rem; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.3); z-index: 1000; text-align: center;">
            <p style="margin-bottom: 0.5rem;">أضف المنيو إلى الشاشة الرئيسية</p>
            <button onclick="installApp()" style="background: white; color: #803a7a; border: none; padding: 0.5rem 1rem; border-radius: 8px; font-weight: 600; cursor: pointer;">تثبيت</button>
